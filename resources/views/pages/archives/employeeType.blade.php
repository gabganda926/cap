...
@section('title','Employee Type - Archives | i-Insure')

@section('archives','active')

@section('aemptype','active')
                            ...
                            <table class="table table-bordered table-striped table-hover dataTable js-basic-example animated lightSpeedIn active">
                                ...
                                <tbody>
                                  @foreach($emptype as $emp)
                                    @if($emp->del_flag == 1)
                                      <tr>
                                          <td><input type="checkbox" id="{{ $emp->emptype_ID }}" name = "del_check" class="filled-in chk-col-red checkCheckbox" data-id = "{{ $emp->emptype_ID }}"/>
                                          <label for="{{ $emp->emptype_ID }}"></label></td> <!--YUNG ID DAPAT NAG IIBA EVERY RECORD -->
                                          <td>{{ $emp->emptype_Name }}</td>
                                          <td>{{ $emp->emptype_Desc }}</td>
                                          ...
                                          <td>
                                            ...
                                            <button type="button" class="rbutton btn bg-green waves-effect" data-toggle="tooltip" data-placement="left" title="Reactivate this employee type" class="btn btn-success btn-xs waves-effect" data-id = "{{ $emp->emptype_ID }}">
                                            <i class="material-icons">autorenew</i>
                                            </button>
                                            <button id = "Delete" type="button" class="btn bg-red waves-effect" data-toggle="tooltip" data-placement="left" title="Delete this employee type permanently">
    ...
    <script>
        ...
        $('.rbutton').click(function(event){
              $.ajax({
                  ...
                  type: 'POST',
                  url: '/admin/archives/employee/type/reactive',
        ...
        $('#reactivate_all').click(function(event){
          event.preventDefault();
              $.ajax({
                  ...
                  type: 'POST',
                  url: '/admin/archives/employee/type/areactive',
